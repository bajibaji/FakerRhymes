...
### 2. 纯 JS 二进制二分检索引擎与大包分片技术 (微秒级响应)
项目采用了自主研发的 **纯 JS 零依赖二进制二分检索引擎**：
* **极限性能**：废弃了原先扫描大文本正则的 $O(N)$ 方案，升级为在类型化数组（`Uint32Array`/`Uint8Array`）上运行的 **二分查找 ($O(\log N)$)** 与 **反向 Key 前缀范围扫描**。**精确检索命中并解码仅需 1 微秒 (0.001ms)**；松散模式下并发 250 个变体查询仅需 **0.4 毫秒**；完全消除了浏览器的卡顿与垃圾回收 (GC) 停顿。
* **Cloudflare 适配**：将 46MB 二进制大包均匀切分为少于 25MB 的 `dict_part1.bin` 与 `dict_part2.bin`，**完美突破 Cloudflare Pages 静态单文件 25MB 上传限制**。
* **极小网络传输**：通过 Gzip / Brotli 压缩，网络传输体积由 38MB 降至 **6MB - 8MB** 左右，实现首次秒开、终身离线秒开。
...
A: 这是因为浏览器缓存了老版本的 main.js 或 Service Worker。请在页面上按下 **Ctrl + F5** 进行强刷，或者在控制台注销 Service Worker 缓存，即可正常加载最新的 **v2.3.13** 二进制版。

**Q: 开发者如何更新词典和运行性能跑分？**
A: 只要本地安装有 Node.js 环境即可一键操作：
* **构建二进制字典**：在根目录下运行 `node js/build_dict.js`，脚本会自动读取 `原始数据/` 目录下的两个原始 txt 文件并编译输出分片包 `dict_part1.bin` 与 `dict_part2.bin`。
* **算法性能跑分**：在根目录下运行 `node js/benchmark_node.js`，可在控制台对当前二进制二分查找和范围扫描算法进行微秒级高逼真性能跑分。
